...
public class crane : MonoBehaviour {
	...
	bool rotateCrane(bool withBox){
		...
		arm.transform.RotateAround(pointOfRotation.renderer.bounds.center,Vector3.up,rot);
	...
	void setDropPos(){
		...
		dropPos.x = dropLocation.x +=pointOfRotation.renderer.bounds.center.x;		
		dropPos.z = dropLocation.z +=pointOfRotation.renderer.bounds.center.z;				
	}
	...
	void findAngle(Vector3 pointA){
		...
		Vector3 pointC = armExtension.renderer.bounds.center;
	...
	void findPlat(Vector3 point){
		point.y=pointOfRotation.renderer.bounds.center.y;
		
		Vector3 plat = hookPlatform.renderer.bounds.center;
		plat.y=pointOfRotation.renderer.bounds.center.y;
		
		
		
		moveTotal=new Vector3(0,Vector3.Distance(point,plat),0);
		if (Vector3.Distance(point,pointOfRotation.renderer.bounds.center)<Vector3.Distance(plat,pointOfRotation.renderer.bounds.center)){
			moveTotal.y*=-1;
		}
	...
	void findHook(Vector3 point){
		...
		Vector3 h=hook.renderer.bounds.center;
		h.y=hook.renderer.bounds.min.y;
	...
	void Update () {	
		...
		if (stage==2){
			if (movePlatform(false)){
				...
				findHook(new Vector3(currentBox.transform.position.x,currentBox.renderer.bounds.max.y,currentBox.transform.position.z));
			}
		}
		if (stage==3){
			if (moveHook(false)){
