...
public class FallingSpikes : MonoBehaviour {
	...
	void Start () {
		...
		Vector3 pos=this.GetComponent<Collider>().bounds.min;
		pos.z+= + (this.GetComponent<Collider>().bounds.size.z)/(spikeWidth*4);
		GameObject s;
		int rand;
		player = GameObject.Find("Player");
		for (int i=0; i<spikeWidth; i++){
			pos.x=this.GetComponent<Collider>().bounds.min.x + (this.GetComponent<Collider>().bounds.size.x)/(spikeDepth*4);
			if (i%2==0){
				pos.x+=(this.GetComponent<Collider>().bounds.size.x)/(spikeDepth*2);
			}
			for (int j=0; j<spikeDepth; j++){
				...
				spikes.Add(s);
				pos.x+=((this.GetComponent<Collider>().bounds.size.x)/spikeDepth);
			}			
			pos.z+=((this.GetComponent<Collider>().bounds.size.z)/spikeWidth);
		}
	...
	void Update () {
		...
		if (Mathf.Sqrt (this.GetComponent<Renderer>().bounds.SqrDistance(playerPos))<fallDist){
