...
public class CeilingCrawler : MonoBehaviour {
	...
	void SpawnOre() {
		...
		if(positiveOre){
			ore.renderer.material = (Material) Resources.Load("Blue", typeof(Material));
			...
			ore.renderer.material = (Material) Resources.Load("Red", typeof(Material));
			ore.AddComponent<Rigidbody>();
			ore.GetComponent<Rigidbody>().mass = 1;
			ore.AddComponent<Magnetism>();
			...
			ore.renderer.material = (Material) Resources.Load("RockStubble", typeof(Material));
